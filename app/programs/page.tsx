...
export default function ProgramsPage() {
  ...
  // Fetch programs when API is configured
  useEffect(() => {
    if (isConfigured) {
      loadPrograms(1);
    }
   ...
   * Load programs data
   */
  const loadPrograms = async (page: number = 1) => {
    setLoading(true);
    setError(null);

    try {
      const response = await fetchPrograms(page, 10);
      ...
      setError(apiError.message || 'Failed to fetch programs data');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-neutral-600">
              Browse and manage academic programs from MyJKKN database
            </p>
          </div>
          {isConfigured && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  {statusMessage}. Please add NEXT_PUBLIC_MYJKKN_API_KEY to your .env.local file to view programs data.
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card variant="default">
            ...
            <div className="mb-6 flex items-center gap-2 text-sm text-neutral-600">
              <span className="font-medium text-brand-green">
                {searchQuery ? filteredPrograms.length : total}
              </span>
              {searchQuery ? 'matching' : 'total'} programs
              {!searchQuery && (
                <>
                  <span>•</span>
            ...
            {loading && (
              <div className="text-center py-16">
                ...
                <p className="text-neutral-600 font-medium">Loading programs...</p>
              </div>
            )}
                    ...
                    <tbody className="divide-y divide-neutral-200">
                      {filteredPrograms.map((program) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <Badge
                              variant={program.is_active ? 'success' : 'default'}
                              size="sm"
                            >
                ...
                {!searchQuery && totalPages > 1 && (
                  <div className="mt-6 flex items-center justify-between border-t border-neutral-200 pt-4">
                    <p className="text-sm text-neutral-600">
                      Showing {programs.length} of {total} results
                    </p>
            ...
            {!loading && !error && filteredPrograms.length === 0 && (
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-brand-green mb-2">
                  {searchQuery ? 'No matching programs' : 'No programs found'}
                </h3>
                <p className="text-neutral-600">
                  {searchQuery
